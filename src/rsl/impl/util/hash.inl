...
namespace rsl
{
	namespace internal
	{
		...
		namespace hash
		{
			...
			hash_bytes(const binary_view bytes, uint64 seed, const uint64 (&secret)[3]) noexcept
			{
...
#undef HASH_INT
	...
	template <hash_mode Mode>
	constexpr id_type hash_bytes(const binary_view bytes) noexcept
	{
	...
	constexpr id_type hash_string(const string_view str) noexcept
	{
		if (is_constant_evaluated())
		{
			...
			const id_type result = hash_bytes<Mode>(binary_view::from_buffer(data, str.size()));
		...
		{
			return hash_bytes<Mode>(binary_view::from_buffer(bit_cast<const byte*>(str.data()), str.size()));
		}
	}
