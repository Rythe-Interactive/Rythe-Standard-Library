...
#define RSL_ERR_MAX_COUNT std::numeric_limits<errid>::max()
    ...
    protected:
        ...
        result_base(error_signal) noexcept : m_errid(error_context::currentError) { error_context::currentError = invalid_err_id; }
    ...
    public:
        result(error_signal) noexcept : result_base(error_signal{}) {}

        template<typename... Args>
            requires(std::constructible_from<T, Args...>)
        constexpr result(Args&&... args) noexcept(std::is_nothrow_constructible_v<T, Args...>) : result_base(), m_value(std::forward<Args>(args)...) {}
    };
