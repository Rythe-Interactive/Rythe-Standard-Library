...
namespace rsl::math::noise
{
	...
	static constexpr vector<integer, 4> primes = vector<integer, 4>(
		static_cast<integer>(501125321), static_cast<integer>(1136930381), static_cast<integer>(1720413743),
	...
	namespace internal
	{
		...
		void xor_impl(
			vector<int_scalar, resultSize>& hash, const vector<int_scalar, size>& value,
			const vector<ints, size>&... others
		)
		{
		...
		static constexpr inline vector<scalar, sizeof...(ints) + 1>
		hash_normalize(int_scalar seed, const vector<int_scalar, size>& value, const vector<ints, size>&... others)
		{
			vector<int_scalar, sizeof...(ints) + 1> hash(seed);

			internal::xor_impl(hash, value, others...);
