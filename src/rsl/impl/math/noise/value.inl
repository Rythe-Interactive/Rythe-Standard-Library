...
namespace rsl::math::noise
{
	namespace internal
	{
		...
		[[nodiscard]] [[rythe_always_inline]] constexpr scalar _value_1d_impl_(int_scalar seed, scalar value) noexcept
		{
			...
			return lerp(
				internal::hash_normalize<scalar>(seed, start),
				internal::hash_normalize<scalar>(seed, start + primes<int_scalar>.x),
				smoothstep(value - static_cast<scalar>(floored))
			);
		}

		template <typename scalar, typename int_scalar = int_least<sizeof(scalar)>>
		[[nodiscard]] [[rythe_always_inline]] constexpr scalar
		_value_1d_impl_(int_scalar seed, scalar value, scalar& derivative) noexcept
		{
			int_scalar floored = ifloor<int_scalar>(value);
			int_scalar start = floored * primes<int_scalar>.x;
			...
			scalar gradStart = internal::hash_normalize<scalar>(seed, start);
			scalar gradEnd = internal::hash_normalize<scalar>(seed, start + primes<int_scalar>.x);
		...
		[[nodiscard]] [[rythe_always_inline]] constexpr scalar
		_value_2d_impl_(int_scalar seed, VectorType value) noexcept
		{
			...
			scalar2 horizontal = lerp(
				internal::hash_normalize<scalar>(seed, start, int_scalar2(start.x, end.y)),
				internal::hash_normalize<scalar>(seed, int_scalar2(end.x, start.y), end), t.x
			);
		...
		template <typename scalar, typename int_scalar = int_least<sizeof(scalar)>, typename VectorType>
		[[nodiscard]] [[rythe_always_inline]] constexpr scalar
		_value_2d_impl_(int_scalar seed, VectorType value, vector<scalar, 2>& derivative) noexcept
		{
			using scalar2 = vector<scalar, 2>;
			using int_scalar2 = vector<int_scalar, 2>;
			...
			scalar2 ac = internal::hash_normalize<scalar>(seed, start, int_scalar2(end.x, start.y));
			scalar2 bd = internal::hash_normalize<scalar>(seed, int_scalar2(start.x, end.y), end);
		...
		template <typename scalar, typename int_scalar = int_least<sizeof(scalar)>, typename VectorType>
		[[nodiscard]] [[rythe_always_inline]] constexpr scalar
		_value_3d_impl_(int_scalar seed, VectorType value) noexcept
		{
			...
			vector<scalar, 4> i = lerp(
				internal::hash_normalize<scalar>(
					seed, start, int_scalar3(start.x, end.y, start.z), int_scalar3(start.x, start.y, end.z),
					int_scalar3(start.x, end.y, end.z)
				),
				internal::hash_normalize<scalar>(
					seed, int_scalar3(end.x, start.y, start.z), int_scalar3(end.x, end.y, start.z),
					int_scalar3(end.x, start.y, end.z), end
				),
				t.x
			);
		...
		template <typename scalar, typename int_scalar = int_least<sizeof(scalar)>, typename VectorType>
		[[nodiscard]] [[rythe_always_inline]] constexpr scalar
		_value_3d_impl_(int_scalar seed, VectorType value, vector<scalar, 3>& derivative) noexcept
		{
			using scalar2 = vector<scalar, 2>;
			using scalar3 = vector<scalar, 3>;
			...
			scalar4 aceg = internal::hash_normalize<scalar>(
				seed, start, int_scalar3(start.x, end.y, start.z), int_scalar3(end.x, start.y, start.z),
				int_scalar3(end.x, end.y, start.z)
			);
			scalar4 bdfh = internal::hash_normalize<scalar>(
				seed, int_scalar3(start.x, start.y, end.z), int_scalar3(start.x, end.y, end.z),
				int_scalar3(end.x, start.y, end.z), end
			);
	...
	[[nodiscard]] constexpr auto value(VectorType&& value, int_least<sizeof(vector_scalar_t<VectorType>)> seed) noexcept
	{
		...
		{
			return internal::_value_1d_impl_(seed, value);
		}
	...
	[[nodiscard]] constexpr auto value(
		VectorType&& value, decay_vector_t<VectorType>& derivative, int_least<sizeof(vector_scalar_t<VectorType>)> seed
	) noexcept
	{
		constexpr size_type dimensions = vector_size_v<VectorType>;
		// using scalar = vector_scalar_t<VectorType>;
		static_assert(
			is_vector_or_scalar_v<VectorType> && dimensions <= 3, "Input type must be a vector of size 3 or lower."
		);
		if constexpr (dimensions == 1)
		{
			return internal::_value_1d_impl_(seed, value, derivative);
		}
		else if constexpr (dimensions == 2)
		{
