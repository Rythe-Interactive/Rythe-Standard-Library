...
namespace rsl::math::noise
{
	namespace detail
	{
		...
		[[nodiscard]] [[rythe_always_inline]] constexpr scalar _value_2d_impl_(int_scalar seed, VectorType value) noexcept
		{
			...
			scalar2 horizontal = lerp(detail::hash_normalize<scalar>(seed, start, int_scalar2(start.x, end.y)), detail::hash_normalize<scalar>(seed, int_scalar2(end.x, start.y), end), t.x);
		...
		[[nodiscard]] [[rythe_always_inline]] constexpr scalar _value_3d_impl_(int_scalar seed, VectorType value) noexcept
		{
			...
			vector<scalar, 4> i = lerp(detail::hash_normalize<scalar>(seed, start, int_scalar3(start.x, end.y, start.z), int_scalar3(start.x, start.y, end.z), int_scalar3(start.x, end.y, end.z)), detail::hash_normalize<scalar>(seed, int_scalar3(end.x, start.y, start.z), int_scalar3(end.x, end.y, start.z), int_scalar3(end.x, start.y, end.z), end), t.x);
	...
	template <typename VectorType>
	[[nodiscard]] constexpr auto value(VectorType&& value, decay_vector_t<VectorType>& derivative, int_least<sizeof(vector_scalar_t<VectorType>)> seed) noexcept
	{
		constexpr size_type dimensions = vector_size_v<VectorType>;
		// using scalar = vector_scalar_t<VectorType>;
