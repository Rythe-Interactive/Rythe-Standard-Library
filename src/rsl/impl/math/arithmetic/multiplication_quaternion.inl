...
namespace rsl::math::detail
{
	...
	struct compute_multiplication<quaternion<Scalar>>
	{
		...
		template<typename AType, typename BType, ::std::enable_if_t<is_quat_v<AType>&& is_vector_v<BType>, bool> = true>
		[[nodiscard]] constexpr static auto compute(AType&& a, BType&& b) noexcept
		{
			using vec_type = make_vector_t<BType>;
			using scalar = typename vec_type::scalar;
			if constexpr (vec_type::size == 3)
			{
				return b * ((a.w * a.w) - dot(a.vec, a.vec)) + static_cast<scalar>(2.0) * a.vec * dot(a.vec, b) + static_cast<scalar>(2.0) * a.w * cross(a.vec, b);
			}
			else if constexpr (vec_type::size == 4)
			{
				return b.vec * ((a.w * a.w) - dot(a.vec, a.vec)) + static_cast<scalar>(2.0) * a.xyz * dot(a.vec, b.vec) + static_cast<scalar>(2.0) * a.w * cross(a.vec, b.vec);
			}
			else
			{
		...
		template<typename AType, typename BType, ::std::enable_if_t<is_quat_v<AType>&& is_scalar_v<BType>, bool> = true>
		[[nodiscard]] constexpr static auto compute(AType&& a, BType&& b) noexcept
		{
			quat_type result;
			result.i = a.i * b;
		...
		template<typename AType, typename BType, ::std::enable_if_t<is_scalar_v<AType>&& is_quat_v<BType>, bool> = true>
		[[nodiscard]] constexpr static auto compute(AType&& a, BType&& b) noexcept
		{
			quat_type result;
			result.i = a.i * b;
