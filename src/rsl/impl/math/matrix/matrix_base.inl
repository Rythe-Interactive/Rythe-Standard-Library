...
namespace rsl::math
{
	...
	template <typename Scalar, size_type RowCount, size_type ColCount>
	[[nodiscard]] constexpr typename matrix<Scalar, RowCount, ColCount>::row_type& matrix<Scalar, RowCount, ColCount>::operator[](size_type i) noexcept
	{
	...
	template <typename Scalar, size_type RowCount, size_type ColCount>
	[[nodiscard]] constexpr const typename matrix<Scalar, RowCount, ColCount>::row_type& matrix<Scalar, RowCount, ColCount>::operator[](size_type i) const noexcept
	{
		rsl_assert_msg((i >= 0) && (i < row_count), "matrix subscript out of range");
		return rows[i];
	}
