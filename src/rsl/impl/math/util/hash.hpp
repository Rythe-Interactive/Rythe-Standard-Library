...
{
    constexpr static ::std::hash<Scalar> hasher{};
    inline rythe_always_inline::std::size_t operator()(const ::rsl::math::vector<Scalar, 1>& vec) const noexcept
    {
        return hasher(vec.x);
    }
};
...
{
    constexpr static ::std::hash<Scalar> hasher{};
    inline rythe_always_inline::std::size_t operator()(const ::rsl::math::vector<Scalar, 2>& vec) const noexcept
    {
        return ::rsl::combine_hash(hasher(vec.x), hasher(vec.y));
    }
};
