...
namespace rsl::math
{
    namespace detail
    {
        ...
        [[nodiscard]] always_inline constexpr Target _round_impl_(T val) noexcept
        {
        ...
        struct compute_round<vector<Scalar, Size>, Mode>
        {
            ...
            [[nodiscard]] always_inline constexpr static auto compute(const value_type& val) noexcept
            {
        ...
        [[nodiscard]] always_inline constexpr auto _auto_adv_round_(T&& val) noexcept
        {
            using value_type = ::std::remove_cvref_t<T>;
    ...
    template<typename Target, round_mode Mode, typename T>
    [[nodiscard]] always_inline constexpr auto adv_round(T&& val) noexcept
    {
        using value_type = ::std::remove_cvref_t<T>;
        if constexpr (is_vector_v<value_type>)
        {
    ...
    template<typename T>
    [[nodiscard]] always_inline constexpr auto round(T&& val) noexcept
    {
        return detail::_auto_adv_round_<round_mode::round>(val);
    }

    template<typename Integer, typename T>
    [[nodiscard]] always_inline constexpr auto iround(T&& val) noexcept
    {
        return adv_round<Integer, round_mode::round>(val);
    }

    template<typename T>
    [[nodiscard]] always_inline constexpr auto uround(T&& val) noexcept
    {
        return adv_round<uint, round_mode::round>(val);
    }

    template<typename T>
    [[nodiscard]] always_inline constexpr auto ceil(T&& val) noexcept
    {
        return detail::_auto_adv_round_<round_mode::ceil>(val);
    }

    template<typename Integer, typename T>
    [[nodiscard]] always_inline constexpr auto iceil(T&& val) noexcept
    {
        return adv_round<Integer, round_mode::ceil>(val);
    }

    template<typename T>
    [[nodiscard]] always_inline constexpr auto uceil(T&& val) noexcept
    {
        return adv_round<uint, round_mode::ceil>(val);
    }

    template<typename T>
    [[nodiscard]] always_inline constexpr auto floor(T&& val) noexcept
    {
        return detail::_auto_adv_round_<round_mode::floor>(val);
    }

    template<typename Integer, typename T>
    [[nodiscard]] always_inline constexpr auto ifloor(T&& val) noexcept
    {
        return adv_round<Integer, round_mode::floor>(val);
    }

    template<typename T>
    [[nodiscard]] always_inline constexpr auto ufloor(T&& val) noexcept
    {
        return adv_round<uint, round_mode::floor>(val);
    }

    template<typename T>
    [[nodiscard]] always_inline constexpr auto trunc(T&& val) noexcept
    {
        return detail::_auto_adv_round_<round_mode::trunc>(val);
    }

    template<typename Integer, typename T>
    [[nodiscard]] always_inline constexpr auto itrunc(T&& val) noexcept
    {
        return adv_round<Integer, round_mode::trunc>(val);
    }

    template<typename T>
    [[nodiscard]] always_inline constexpr auto utrunc(T&& val) noexcept
    {
