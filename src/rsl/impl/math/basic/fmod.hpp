...
namespace rsl::math
{
    namespace detail
    {
        ...
        struct compute_mod
        {
            ...
            [[nodiscard]] inline always_inline static value_type computef(const value_type& val, Scalar m) noexcept
            {
                value_type result;
            ...
            [[nodiscard]] inline always_inline static value_type computef(const value_type& val, const value_type& m) noexcept
            {
                value_type result;
                for (size_type i = 0; i < size; i++)
        ...
        struct compute_mod<Scalar, 1u>
        {
            ...
            [[nodiscard]] inline always_inline static Scalar computef(Scalar val, Scalar m) noexcept
            {
    ...
    template<typename T>
    [[nodiscard]] inline always_inline static auto fmod(const T& val, const T& m)
    {
    ...
    [[nodiscard]] inline always_inline static auto fmod(const vec_type& val, typename vec_type::scalar m)
    {
    ...
    template<typename T>
    [[nodiscard]] inline always_inline static auto mod(const T& val, const T& m)
    {
        if constexpr (is_vector_v<T>)
        {
    ...
    template<typename vec_type, std::enable_if_t<is_vector_v<vec_type>, bool> = true>
    [[nodiscard]] inline always_inline static auto mod(const vec_type& val, typename vec_type::scalar m)
    {
    ...
    template<typename vec_type, std::enable_if_t<is_vector_v<vec_type>, bool> = true>
    [[nodiscard]] inline always_inline auto operator%(const vec_type& val, typename vec_type::scalar m)
    {
        return mod(val, m);
    }

    template<typename vec_type, std::enable_if_t<is_vector_v<vec_type>, bool> = true>
    [[nodiscard]] inline always_inline auto operator%(const vec_type& val, const vec_type& m)
    {
        return mod(val, m);
    }
}
