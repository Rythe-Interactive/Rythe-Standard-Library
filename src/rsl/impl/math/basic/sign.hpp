...
namespace rsl::math
{
	...
	namespace detail
	{
		...
		struct compute_signum<vector<Scalar, Size>>
		{
			...
			[[nodiscard]] rythe_always_inline constexpr static vec_type compute(const vec_type& val) noexcept
			{
				vec_type result;
			...
			[[nodiscard]] rythe_always_inline constexpr static vec_type compute_sign(const vec_type& val) noexcept
			{
				vec_type result;
				for (size_type i = 0; i < size; i++)
					result[i] = sign(val[i]);
		...
		struct compute_signum<vector<Scalar, 1>>
		{
			...
			[[nodiscard]] rythe_always_inline constexpr static Scalar compute(const vec_type& val) noexcept
			{
				return signum(val[0]);
			}

			[[nodiscard]] rythe_always_inline constexpr static Scalar compute_sign(const vec_type& val) noexcept
			{
				return sign(val[0]);
			}
		};
