...
namespace rsl::math::detail
{
	...
	struct compute_map<vector<Scalar, Size>>
	{
		...
		[[nodiscard]] constexpr static auto compute(T&& value, InMin&& inMin, InMax&& inMax, OutMin&& outMin, OutMax&& outMax) noexcept
		{
			vec_type result;
	...
	struct compute_map<vector<Scalar, 1u>>
	{
		...
		template <typename T, typename InMin, typename InMax, typename OutMin, typename OutMax>
		[[nodiscard]] constexpr static Scalar compute(T&& value, InMin&& inMin, InMax&& inMax, OutMin&& outMin, OutMax&& outMax) noexcept
		{
