...
namespace rsl::math
{
    ...
    [[nodiscard]] always_inline auto inverse(const vec_type& v) noexcept;

    template<typename quat_type, ::std::enable_if_t<is_quat_v<quat_type>, bool> = true>
    [[nodiscard]] always_inline auto inverse(const quat_type& q) noexcept;

    template<typename mat_type, ::std::enable_if_t<is_matrix_v<mat_type>, bool> = true>
    [[nodiscard]] always_inline auto inverse(const mat_type& m) noexcept;
}
