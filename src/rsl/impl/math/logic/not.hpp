...
namespace rsl::math
{
	namespace internal
	{
		...
		struct compute_not<Scalar, 1u>
		{
			...
			[[nodiscard]] constexpr static bool compute(Scalar value) noexcept { return !value; }
		};
	} // namespace internal
	...
	{
		return internal::compute_not<typename vec_type::scalar, vec_type::size>::compute(value);
	}

	template <typename Scalar, std::enable_if_t<!is_vector_v<Scalar>, bool> = true>
	[[nodiscard]] constexpr auto not(Scalar value) noexcept
	{
		return internal::compute_not<Scalar, 1u>::compute(value);
	}
