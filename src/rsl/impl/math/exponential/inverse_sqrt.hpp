...
namespace rsl::math
{
    namespace detail
    {
        ...
        struct compute_inv_sqrt
        {
            ...
            always_inline static value_type compute(const value_type& v) noexcept
            {
                value_type result;
        ...
        struct compute_inv_sqrt<Scalar, 1u>
        {
            ...
            always_inline static Scalar compute(Scalar v) noexcept
            {
    ...
    always_inline static auto inverse_sqrt(const vec_type& v) noexcept
    {
        return detail::compute_inv_sqrt<typename vec_type::scalar, vec_type::size>::compute(v);
    }

    template<typename Scalar>
    always_inline static Scalar inverse_sqrt(Scalar v) noexcept
    {
