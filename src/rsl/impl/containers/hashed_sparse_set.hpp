...
namespace rsl
{
	...
	public:
		...
		[[nodiscard]] const_reverse_iterator rbegin() const noexcept { return m_dense.crbegin() + (m_dense.size() - m_size); }
		[[nodiscard]] const_reverse_iterator crbegin() const noexcept { return m_dense.crbegin() + (m_dense.size() - m_size); }
...
#pragma region count
		 ...
		 */
		[[nodiscard]] size_type count(value_const_reference val) const
		{
			return contains(val);
		}
		 ...
		 * @ref legion::core::sparse_map::contains
		 */
		[[nodiscard]] size_type count(value_type&& val) const
		{
			return contains(val);
		}
#pragma endregion
