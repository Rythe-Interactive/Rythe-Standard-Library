...
namespace rsl
{
	...
	constexpr array_view<T, Iter, ConstIter>::array_view(It first, It last)
	...
	constexpr array_view<T, Iter, ConstIter>::array_view(value_type (&arr)[N]) noexcept : m_src(arr), m_count(N)
	{
	...
	template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter>
	constexpr array_view<T, Iter, ConstIter>::array_view(const array_view& other) noexcept : m_src(other.m_src), m_count(other.m_count) {}

	template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter>
	constexpr array_view<T, Iter, ConstIter>::array_view(value_type& src) noexcept : m_src(&src), m_count(1) {}

	template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter>
	constexpr array_view<T, Iter, ConstIter>::operator array_view<const T, const_iterator_type>() noexcept requires (!is_const_v<T>)
	{
	...
	template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter>
	constexpr typename array_view<T, Iter, ConstIter>::const_reverse_iterator_type array_view<T, Iter, ConstIter>::rbegin()
	const noexcept
	{
		return crbegin();
	}
	...
	template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter>
	constexpr typename array_view<T, Iter, ConstIter>::const_reverse_iterator_type array_view<T, Iter, ConstIter>::crbegin()
	const noexcept
	{
	...
	template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter>
	constexpr typename array_view<T, Iter, ConstIter>::const_reverse_iterator_type array_view<T, Iter, ConstIter>::rend()
	const noexcept
	{
		return crend();
	}
	...
	template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter>
	constexpr typename array_view<T, Iter, ConstIter>::const_reverse_iterator_type array_view<T, Iter, ConstIter>::crend()
	const noexcept
	{
	...
	template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter>
	constexpr array_view<T, Iter, ConstIter> array_view<T, Iter, ConstIter>::subview(size_type pos, size_type n) const
	{
		...
		return array_view<T, Iter, ConstIter>(m_src + pos, n);
	}

	//TODO(Rowan): Implement a better search algo
	template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter>
	constexpr size_type find_first_of(array_view<const T, Iter> str, array_view<const T, Iter> key, size_type pos) noexcept
	{
	...
	template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter>
	constexpr size_type find_first_not_of(array_view<const T, Iter> str, array_view<const T, Iter> key, size_type pos) noexcept
	{
		for (auto iter = str.begin() + pos; iter != str.end(); ++iter)
		{
			bool found = true;
	...
	template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter>
	constexpr size_type find_last_of(array_view<const T, Iter> str, array_view<const T, Iter> key, size_type pos) noexcept
	{
	...
	template <typename T, contiguous_iterator Iter, contiguous_iterator ConstIter>
	constexpr size_type find_first_of(array_view<const T, Iter> str, const T& key, size_type pos) noexcept
	{
		return find_first_of(str, array_view(&key, 1), pos);
	}

	template <typename T, contiguous_iterator Iter>
	constexpr size_type find_first_of([[maybe_unused]] array_view<const T, Iter> str, [[maybe_unused]] array_view<const T, Iter> key, [[maybe_unused]] size_type pos) noexcept
	{
	...
	template <typename T, contiguous_iterator Iter>
	constexpr size_type find_first_not_of([[maybe_unused]] array_view<const T, Iter> str, [[maybe_unused]] array_view<const T, Iter> key, [[maybe_unused]] size_type pos) noexcept
	{
		rsl_assert_unimplemented();
		return 0;
	}
	...
	template <typename T, contiguous_iterator Iter>
	constexpr size_type find_last_of([[maybe_unused]] array_view<const T, Iter> str, [[maybe_unused]] array_view<const T, Iter> key, [[maybe_unused]] size_type pos) noexcept
	{
		rsl_assert_unimplemented();
		return 0;
	}
	...
	template <typename T, contiguous_iterator Iter>
	constexpr size_type find_last_not_of([[maybe_unused]] array_view<const T, Iter> str, [[maybe_unused]] array_view<const T, Iter> key, [[maybe_unused]] size_type pos) noexcept
	{
		rsl_assert_unimplemented();
		return 0;
	}
	...
	template <typename T, contiguous_iterator Iter>
	constexpr size_type find_first_of([[maybe_unused]] array_view<const T, Iter> str, [[maybe_unused]] const T& key, [[maybe_unused]] size_type pos) noexcept
	{
		rsl_assert_unimplemented();
		return 0;
	}
	...
	constexpr size_type find_first_not_of(array_view<const T, Iter> str, const C& key, size_type pos) noexcept
	{
		return find_first_not_of(str, array_view(&key, 1), pos);
	}

	template <typename T, contiguous_iterator Iter, same_as<T> C>
	constexpr size_type find_last_of(array_view<const T, Iter> str, const C& key, size_type pos) noexcept
	{
		return find_last_of(str, array_view(&key, 1), pos);
	}

	template <typename T, contiguous_iterator Iter, same_as<T> C>
	constexpr size_type find_last_not_of(array_view<const T, Iter> str, const C& key, size_type pos) noexcept
	{
		return find_last_not_of(str, array_view(&key, 1), pos);
	}
