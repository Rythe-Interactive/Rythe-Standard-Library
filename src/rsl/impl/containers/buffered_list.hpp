...
namespace rsl {
    ...
    public:
        ...
        constexpr buffered_list(const buffered_list& src) noexcept(copy_construct_noexcept) : m_size(validate_size(src.m_size)) {
            copy_construct_from_unsafe_impl(0, m_size, src.m_buffer);
        }

        constexpr buffered_list(buffered_list&& src) noexcept(move_construct_noexcept) : m_size(validate_size(src.m_size)) {
            move_construct_from_unsafe_impl(0, m_size, src.m_buffer);
        }

        template<size_type N>
        constexpr buffered_list(const value_type(&arr)[N]) noexcept(copy_construct_noexcept) : m_size(validate_size(N)) {
            copy_construct_from_unsafe_impl(0, m_size, arr);
        }

        template<size_type N>
        constexpr buffered_list(value_type(&&arr)[N]) noexcept(move_construct_noexcept) : m_size(validate_size(N)) {
            move_construct_from_unsafe_impl(0, m_size, arr);
        }

        constexpr buffered_list(view_type src) noexcept(copy_construct_noexcept) : m_size(validate_size(src.size())) {
