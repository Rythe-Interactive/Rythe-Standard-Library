...
namespace rsl
{
    ...
    public:
        ...
        rythe_always_inline time_point_type start_point()  const noexcept { return time_point_type{ m_start.time_since_epoch() }; }
        rythe_always_inline time_point_type current_point()  const noexcept { return time_point_type{ clock_type::now() }; }
        rythe_always_inline span_type elapsed_time() const noexcept { return span_type{ clock_type::now() - m_start }; }

        rythe_always_inline void fast_forward(span_type time) noexcept(std::is_arithmetic_v<time_type>) { m_start -= std::chrono::duration_cast<typename clock_type::duration>(time.duration); }
