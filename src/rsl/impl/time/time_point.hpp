...
namespace rsl
{
	...
	private:
		...
		[[nodiscard]] constexpr days_duration<T> d_dur() const noexcept(std::is_arithmetic_v<rep>)
		{
			return std::chrono::duration_cast<days_duration<T>>(std::chrono::abs(duration));
		}
		template <time_duration_rep T>
		[[nodiscard]] constexpr hours_duration<T> h_dur() const noexcept(std::is_arithmetic_v<rep>)
		{
	...
	public:
		...
		constexpr explicit time_point(const duration_type& other) noexcept(std::is_arithmetic_v<rep>) /* strengthened */
			: duration(other)
		{
		}
		constexpr explicit time_point(const tp_t& other) noexcept(std::is_arithmetic_v<rep>) /* strengthened */
		...
		constexpr time_point(const time_point<clock_t, other_duration>& other)
			noexcept(std::is_arithmetic_v<rep> && std::is_arithmetic_v<typename other_duration::rep>) /* strengthened */
		...
		template <time_duration_rep T>
		[[nodiscard]] constexpr T hours() const noexcept(std::is_arithmetic_v<rep> && std::is_arithmetic_v<T>)
		{
			return h_dur<T>().count();
		}
		template <time_duration_rep T>
		[[nodiscard]] constexpr T minutes() const noexcept(std::is_arithmetic_v<rep> && std::is_arithmetic_v<T>)
		{
			return m_dur<T>().count();
		}
		template <time_duration_rep T>
		[[nodiscard]] constexpr T seconds() const noexcept(std::is_arithmetic_v<rep> && std::is_arithmetic_v<T>)
		{
			return s_dur<T>().count();
		}
		template <time_duration_rep T>
		[[nodiscard]] constexpr T milliseconds() const noexcept(std::is_arithmetic_v<rep> && std::is_arithmetic_v<T>)
		{
			return milli_dur<T>().count();
		}
		template <time_duration_rep T>
		[[nodiscard]] constexpr T microseconds() const noexcept(std::is_arithmetic_v<rep> && std::is_arithmetic_v<T>)
		{
			return micro_dur<T>().count();
		}
		template <time_duration_rep T>
		[[nodiscard]] constexpr T nanoseconds() const noexcept(std::is_arithmetic_v<rep> && std::is_arithmetic_v<T>)
		{
